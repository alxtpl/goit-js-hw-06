...
ctrlNode.children[1].addEventListener('click', () => { boxesNode.insertAdjacentHTML('afterbegin', [...newArr].join('')) });
...
function createBoxes(amount) {
    ...
    for (let index = 0; index < amount.currentTarget.value; index++) {
        ...
        width += 10;
        heigh += 10;
        newArr[index] = `<div style="width: ${width}px; height:${heigh}px; background-color: ${getRandomHexColor()}; z-index: ${index};"></div>`;
        // console.log(newArr);
    }
...
function destroyBoxes() {
    boxesNode.childNodes.forEach(item => {
        item.remove()
            // console.log(item);
    })
}